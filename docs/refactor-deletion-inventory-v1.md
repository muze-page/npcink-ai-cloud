...
lease; P3-B4C2 inventory reconciliation remains pending. B4B3 removes the dead
...
| MED-04 | P3-B4C1b removes the old `media_derivatives` cleanup helper and implements one `MediaArtifact` TTL purge with database claim truth, artifact-first delivery locking, revocation, delete-outside-transaction, fenced finalize, retry, and five-count cadence evidence. | consolidate, partially implemented | `MediaArtifact` owns TTL purge and consumer acknowledgement coordination. P3-B4C2 must add bounded artifact-store inventory versus database inventory reconciliation after a safety window; it must not consume Session-local C1a quarantine. | P3 | C1b model/migration, lifecycle, delivery/ACK race, projection privacy, cadence evidence, and static deletion tests pass. C2 inventory tests, C3 PostgreSQL real-concurrency/PG16 migration validation, and B4D real WordPress local-import smoke remain required. |
