...
fencing, while B4C2b destructive orphan cleanup remains pending. B4B3 removes the dead
...
| MED-04 | P3-B4C1b removes the old `media_derivatives` cleanup helper and implements one `MediaArtifact` TTL purge with database claim truth, artifact-first delivery locking, revocation, delete-outside-transaction, fenced finalize, retry, and five-count cadence evidence. P3-B4C2a adds strict local-volume inventory, read-only two-direction reconciliation, publication fencing, and independent aggregate cadence evidence. | consolidate, partially implemented | `MediaArtifact` owns TTL purge and consumer acknowledgement coordination. C2a never consumes Session-local C1a quarantine and never deletes inventory observations. P3-B4C2b must add durable two-pass claims, exclusive fencing, final conditional reference checks, and fenced deletion/finalization before automatic orphan cleanup is allowed. | P3 | C1b model/migration, lifecycle, delivery/ACK race, projection privacy, cadence evidence, and static deletion tests pass. C2a inventory, reconciliation, cross-process fence, no-delete, config, and cadence tests pass. C2b destructive cleanup, C3 PostgreSQL real-concurrency/PG16 migration validation, and B4D real WordPress local-import smoke remain required. |
