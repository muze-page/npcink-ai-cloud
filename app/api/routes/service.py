...
class RecognitionModelAnnotationPayload(BaseModel):
...
def _get_commercial_service(request: Request) -> CommercialService:
    ...
    return CommercialService(services.settings.database_url)
...
def _get_catalog_service(request: Request) -> CatalogService:
    ...
    return CatalogService(
        ...
        recognition_review_providers=build_recognition_review_provider_adapters(services.settings),
        recognition_evidence_snapshot_path=services.settings.recognition_evidence_snapshot_path,
    )
...
def _serialize_review_only_recognition_list_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in items:
        ...
        sanitized_annotation = annotation
        if isinstance(annotation, dict):
            sanitized_annotation = {
                key: annotation.get(key)
                for key in ("review_status", "manual_tags", "operator_notes", "updated_at")
            }
        sanitized_items.append(
            {
                **item,
                "annotation": sanitized_annotation,
            }
        )
...
async def list_admin_provider_connections(request: Request) -> Any:
    ...
    if auth is not None:
        return auth
    result = _get_catalog_service(request).list_admin_provider_connections()
    return build_envelope(
        status="ok",
        message="admin provider connections loaded",
        data=result,
        revision="m6",
    )


@router.get("/admin/providers/{connection_id}")
async def get_admin_provider_connection(
    request: Request,
    connection_id: str,
) -> Any:
    auth = await authorize_internal_request(request, require_idempotency=False)
    if auth is not None:
        return auth
    result = _get_catalog_service(request).get_admin_provider_connection(connection_id)
    if result is None:
        return JSONResponse(
            status_code=404,
            content=build_envelope(
                status="error",
                error_code="catalog.admin_provider_connection_not_found",
                message="admin provider connection not found",
    ...
    return build_envelope(
        status="ok",
        message="admin provider connection loaded",
        data=result,
        revision="m6",
    )
...
async def list_admin_models(
    ...
    deprecated_only: bool = Query(default=False),
    page: int = Query(default=1, ge=1, le=10000),
    per_page: int = Query(default=50, ge=1, le=200),
    sort_by: str = Query(default="provider_id"),
    ...
    if auth is not None:
        return auth
    try:
        result = _get_catalog_service(request).list_admin_models(
        ...
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="catalog.admin_models_invalid_filter",
                message=str(error),
                revision="m6",
            ),
        )
...
async def get_admin_model(
    ...
    if auth is not None:
        return auth
    result = _get_catalog_service(request).get_admin_model(model_id)
    if result is None:
        return JSONResponse(
            status_code=404,
            content=build_envelope(
...
async def upsert_admin_model_annotation(
    request: Request,
    model_id: str,
    payload: CatalogModelAnnotationPayload,
) -> Any:
    auth = await authorize_internal_request(request, require_idempotency=True)
    if auth is not None:
        return auth
...
async def list_admin_recognition_models(
    request: Request,
    provider_id: str | None = Query(default=None),
    search: str | None = Query(default=None),
    review_status: str | None = Query(default=None),
    in_hosted_catalog: bool | None = Query(default=None),
    ...
    if auth is not None:
        return auth
    try:
        result = _get_catalog_service(request).list_admin_recognition_models(
            provider_id=provider_id,
            search=search,
        ...
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="catalog.admin_recognition_invalid_filter",
                message=str(error),
                revision="m6",
            ),
        )
    return build_envelope(
        status="ok",
        message="admin recognition models loaded",
        data=_serialize_review_only_recognition_list_payload(result),
        revision="m6",
    )


@router.get("/admin/recognition/{provider_id}/{model_id:path}")
async def get_admin_recognition_model(
    request: Request,
    provider_id: str,
    model_id: str,
) -> Any:
    ...
    if auth is not None:
        return auth
    result = _get_catalog_service(request).get_admin_recognition_model(
        provider_id=provider_id,
        model_id=model_id,
    )
    if result is None:
    ...
    return build_envelope(
        status="ok",
        message="admin recognition model loaded",
        data=result,
        revision="m6",
    )
...
async def upsert_admin_provider_connection(
    request: Request,
    connection_id: str,
    ...
    if connection_id != payload.connection_id:
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="catalog.admin_provider_connection_invalid",
                message="connection_id path and payload must match",
                revision="m6",
            ),
        )
    try:
        result = _get_catalog_service(request).upsert_admin_provider_connection(
        ...
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="catalog.admin_provider_connection_unavailable",
                message=str(error),
                revision="m6",
            ),
        )
    ...
    audit_event = _get_commercial_service(request).record_service_audit_event(
        ...
        scope_id=payload.connection_id,
        payload_json={"connection": result},
    )
    return build_envelope(
        status="ok",
        message="admin provider connection saved",
        data=_merge_receipt(
            result,
            _build_operator_receipt(
                event_kind="provider_connection.upsert",
                ...
                scope_id=payload.connection_id,
                outcome="succeeded",
                effective_summary=f"Provider connection {payload.connection_id} is now saved with the latest source configuration.",
...
async def test_admin_provider_connection(
    ...
    if auth is not None:
        return auth
    try:
        result = _get_catalog_service(request).test_admin_provider_connection(connection_id)
    except ValueError as error:
        return JSONResponse(
            status_code=400,
            content=build_envelope(
        ...
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="catalog.admin_provider_connection_test_failed",
                message=str(error),
                revision="m6",
            ),
        )
    ...
    audit_event = _get_commercial_service(request).record_service_audit_event(
        ...
        payload_json={"test_result": result},
    )
    return build_envelope(
        status="ok",
        message="admin provider connection tested",
        data=_merge_receipt(
            result,
            _build_operator_receipt(
                event_kind="provider_connection.test",
                ...
                scope_id=connection_id,
                outcome="succeeded",
                effective_summary=f"Connection test for {connection_id} completed with the latest provider health evidence.",
                audit_event=audit_event,
            ),
        ),
        revision="m6",
...
async def sync_admin_provider_connection_catalog(
    request: Request,
    connection_id: str,
) -> Any:
    auth = await authorize_internal_request(request, require_idempotency=True)
    if auth is not None:
        return auth
    try:
        result = _get_catalog_service(request).sync_admin_provider_connection_catalog(
            connection_id
        )
    except ValueError as error:
        return JSONResponse(
        ...
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="catalog.admin_provider_connection_sync_failed",
                message=str(error),
                revision="m6",
            ),
        )
    ...
    audit_event = _get_commercial_service(request).record_service_audit_event(
        ...
        scope_kind="provider_connection",
        scope_id=connection_id,
        payload_json={"sync_result": result},
    )
    return build_envelope(
        status="ok",
        message="admin provider connection synced",
        data=_merge_receipt(
            result,
            _build_operator_receipt(
                event_kind="provider_connection.sync",
                ...
                scope_id=connection_id,
                outcome="succeeded",
                effective_summary=f"Hosted catalog sync for {connection_id} finished and the latest execution revision is now visible.",
                audit_event=audit_event,
            ),
        ),
        revision="m6",
...
@router.post("/admin/providers/{connection_id}/promote")
async def promote_admin_provider_connection_execution_revision(
    request: Request,
    connection_id: str,
) -> Any:
    auth = await authorize_internal_request(request, require_idempotency=True)
    if auth is not None:
        return auth
    try:
        result = _get_catalog_service(request).promote_admin_provider_connection_execution_revision(
            connection_id
        )
    except ValueError as error:
        return JSONResponse(
    ...
    audit_event = _get_commercial_service(request).record_service_audit_event(
        audit_context=_build_audit_context(request),
        event_kind="provider_connection.promote",
        outcome="succeeded",
        scope_kind="provider_connection",
        scope_id=connection_id,
        payload_json={"promotion": result},
    )
    return build_envelope(
        status="ok",
        message="admin provider connection promoted",
        data=_merge_receipt(
            result,
            _build_operator_receipt(
                event_kind="provider_connection.promote",
                ...
                scope_id=connection_id,
                outcome="succeeded",
                effective_summary=f"Execution revision for {connection_id} is now promoted for downstream hosted review.",
                audit_event=audit_event,
            ),
        ),
        revision="m6",
...
async def upsert_admin_recognition_annotation(
    request: Request,
    provider_id: str,
    model_id: str,
    payload: RecognitionModelAnnotationPayload,
) -> Any:
    auth = await authorize_internal_request(request, require_idempotency=True)
    if auth is not None:
        return auth
