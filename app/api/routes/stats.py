...
def _get_prompt_advisor_filters(
    prompt_id: str = Query(default="", max_length=191),
    ability_id: str = Query(default="", max_length=191),
    current_variant: str = Query(default="current", max_length=64),
    current_version: int = Query(default=0, ge=0, le=2147483647),
    candidate_variant: str = Query(default="candidate", max_length=64),
    candidate_version: int = Query(default=0, ge=0, le=2147483647),
    range: str = Query(default="24h", max_length=16),
    start_gmt: str = Query(default="", max_length=32),
    end_gmt: str = Query(default="", max_length=32),
) -> dict[str, Any]:
...
def _get_preset_advisor_filters(
    preset_id: str = Query(default="", max_length=191),
    default_preset_id: str = Query(default="", max_length=191),
    ability_id: str = Query(default="", max_length=191),
    range: str = Query(default="24h", max_length=16),
    start_gmt: str = Query(default="", max_length=32),
    end_gmt: str = Query(default="", max_length=32),
...
@router.get("/v1/prompt/advisor/recommendation")
async def get_prompt_advisor_recommendation_summary(
    request: Request,
    filters: dict[str, Any] = Depends(_get_prompt_advisor_filters),
) -> Any:
    auth = await authorize_public_request(
        request,
        require_idempotency=False,
    ...
    try:
        result = _get_usage_service(request).get_prompt_advisor_recommendation_summary(
            site_id=auth.site_id,
            filters=filters,
        )
    except ValueError as error:
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="stats.invalid_prompt_advisor_window",
                message=str(error),
                data=filters,
                revision="m1",
            ),
    ...
    return build_envelope(
        status="ok",
        message="prompt advisor recommendation summary loaded",
        data=result,
        revision="m1",
    )


@router.get("/v1/prompt/eval/recommendation")
async def get_prompt_eval_recommendation_summary(
    request: Request,
    filters: dict[str, Any] = Depends(_get_prompt_advisor_filters),
) -> Any:
    auth = await authorize_public_request(
    ...
    try:
        result = _get_usage_service(request).get_prompt_eval_recommendation_summary(
            site_id=auth.site_id,
            filters=filters,
        )
    except ValueError as error:
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="stats.invalid_prompt_eval_window",
                message=str(error),
                data=filters,
                revision="m1",
            ),
    ...
    return build_envelope(
        status="ok",
        message="prompt eval recommendation summary loaded",
        data=result,
        revision="m1",
    )


@router.get("/v1/prompt/canary/recommendation")
async def get_prompt_canary_recommendation_summary(
    request: Request,
    filters: dict[str, Any] = Depends(_get_prompt_advisor_filters),
) -> Any:
    auth = await authorize_public_request(
    ...
    try:
        result = _get_usage_service(request).get_prompt_canary_recommendation_summary(
            site_id=auth.site_id,
            filters=filters,
        )
    except ValueError as error:
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="stats.invalid_prompt_canary_window",
                message=str(error),
                data=filters,
                revision="m1",
            ),
    ...
    return build_envelope(
        status="ok",
        message="prompt canary recommendation summary loaded",
        data=result,
        revision="m1",
    )


@router.get("/v1/prompt/upgrade/recommendation")
async def get_prompt_upgrade_recommendation_summary(
    request: Request,
    filters: dict[str, Any] = Depends(_get_prompt_advisor_filters),
) -> Any:
    auth = await authorize_public_request(
    ...
    try:
        result = _get_usage_service(request).get_prompt_upgrade_recommendation_summary(
            site_id=auth.site_id,
            filters=filters,
        )
    except ValueError as error:
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="stats.invalid_prompt_upgrade_window",
                message=str(error),
                data=filters,
                revision="m1",
            ),
    ...
    return build_envelope(
        status="ok",
        message="prompt upgrade recommendation summary loaded",
        data=result,
        revision="m1",
    )


@router.get("/v1/preset/advisor/recommendation")
async def get_preset_advisor_recommendation_summary(
    request: Request,
    filters: dict[str, Any] = Depends(_get_preset_advisor_filters),
) -> Any:
    auth = await authorize_public_request(
        request,
        require_idempotency=False,
    ...
    try:
        result = _get_usage_service(request).get_preset_advisor_recommendation_summary(
            site_id=auth.site_id,
            filters=filters,
        )
    except ValueError as error:
        return JSONResponse(
            ...
            content=build_envelope(
                status="error",
                error_code="stats.invalid_preset_advisor_window",
                message=str(error),
    ...
    return build_envelope(
        status="ok",
        message="preset advisor recommendation summary loaded",
        data=result,
        revision="m1",
    )
